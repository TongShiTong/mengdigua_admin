...
<!--          </el-form-item>-->
          ...
          <el-form-item v-if="0&&goodstype!=4&&sbc_id" :rules="[{ required: true}]" label="商品分类">
            <el-tree
              ref="tree"
              :data="Gorys"
              :props="defaultProps"
          ...
          <el-form-item v-if="(goodstype!=4)&&(shop_id==0||shop_id=='')" :rules="[{ required: true}]" label="来源店铺" style="width:300px;" @click.native="openShops()">
            <el-input v-model="storeName" placeholder="点击选择店铺"/>
          </el-form-item>

          <el-form-item :rules="[{ required: true}]" label="标题" style="width:600px;">
            <el-input v-model="goodsname" placeholder="请输入标题"/>
          </el-form-item>
          ...
          <el-form-item label="短视频 (mp4) (<5M)" style="width:900px;">
          ...
          <el-form-item label="商品货号" style="width:300px;">
            <el-input v-model="goodsNum" placeholder="请输入货号"/>
          </el-form-item>

          <el-form-item label="商品标签" style="width:300px;">
            <el-input v-model="item_label" placeholder="请输入商品标签"/>
          </el-form-item>

          <el-form-item label="商品简介" style="width:600px;">
            <el-input v-model="goodsIntro" type="textarea"/>
          </el-form-item>

          <el-form-item label="产地" style="width:300px;">
            <el-input v-model="place" placeholder="请输入产地"/>
          </el-form-item>

          <el-form-item label="产量" style="width:300px;">
            <el-input v-model="output" placeholder="请输入产量"/>
          </el-form-item>

          <el-form-item label="品质" style="width:300px;">
            <el-input v-model="quality" placeholder="请输入品质"/>
          </el-form-item>

          <el-form-item label="规模" style="width:300px;">
            <el-input v-model="scale" placeholder="请输入规模"/>
          </el-form-item>

          <el-form-item label="排序" style="width:300px;">
            <el-input v-model="goodsOrd" placeholder="请输入排序值"/>
          </el-form-item>

          <el-form-item label="优惠金额" style="width:300px;">
            <el-input v-model="discount" placeholder="请输入优惠金额"/>
          </el-form-item>

          <el-form-item :rules="[{ required: true}]" label="原价" style="width:300px;">
            <el-input v-model="prePrice" placeholder="请输入原价"/>
          </el-form-item>

          <el-form-item :rules="[{ required: true}]" label="售价" style="width:300px;">
            <el-input v-model="mainPrice" placeholder="请输入售价"/>
          </el-form-item>

          <el-form-item :rules="[{ required: true}]" label="库存" style="width:300px;">
            <el-input v-model="goodsStock" placeholder="请输入库存"/>
          </el-form-item>
          ...
          <el-form-item v-if="goodstype!=4" :rules="[{ required: true}]" label="是否自营" style="width:300px;">
            <el-radio v-model="is_support" label="1">是</el-radio>
            <el-radio v-model="is_support" label="0">否</el-radio>
          </el-form-item>
          ...
          <el-form-item v-if="goodstype!=4" :rules="[{ required: true}]" label="是否参与分佣" style="width:300px;">
            <el-radio v-model="is_bonus" label="1" @change="isBonus">是</el-radio>
            <el-radio v-model="is_bonus" label="0" @change="isBonus">否</el-radio>
          </el-form-item>

          <el-form-item v-if="has_sub_commission" :rules="[{ required: true}]" label="分佣比列" style="width:300px;">
            <el-input v-model="sub_commission" type="number" max="100" min="0" placeholder="请输入分佣比列"/>
            <span style="color:red;">分佣比列为0~100的正整数</span>
          </el-form-item>

          <el-form-item :rules="[{ required: true}]" label="快递运费" style="width:500px;">
            <div style="display:flex;alignItems:center">
              <el-radio v-model="expressType" label="1" @change="oneExp">统一运费</el-radio>
              &nbsp;&nbsp;&nbsp;
              <el-input v-model="expressPrice" placeholder="请输入运费"/>
            </div>
            <div style="height:20px;"/>
            <div style="display:flex;alignItems:center">
              <el-radio v-model="expressType" label="2">运费模板</el-radio>
              &nbsp;&nbsp;&nbsp;
              <el-select v-model="expvalue" placeholder="请选择运费模板" @change="expItem">
                <el-option
            ...
            <div class="guigeBox">
              <div v-for="(item,index) in goodsMSpec" :key="index" style="marginTop:10px;">
                <el-form-item :label="index==0?'主规格名':'规格名'">
                  ...
                  <el-checkbox v-if="index==0" v-model="specIMG" label="添加主规格图片 (190px*190px)" border/>
                  <el-button v-else type="warning" plain @click="delMainSpc(index)">删除该规格</el-button>
                </el-form-item>
          ...
          <el-form-item label="商品详情富文本" style="width:900px;">
            <el-upload
              ...
              :on-success="handleAvatarSuccessRich1"
              style="display:none"
              multiple
              list-type="picture-card">
              <i class="el-icon-plus imgupload1 "/>
            </el-upload>
            <quill-editor
              ref="QuillEditor"
              v-model="content"
          ...
          <el-form-item label="购买须知富文本" style="width:900px;">
            <el-upload
              ...
              :on-preview="handlePictureCardPreview"
              :on-remove="handleRemove"
              :on-success="handleAvatarSuccessRich2"
              style="display:none"
              multiple
              list-type="picture-card">
              <i class="el-icon-plus imgupload2 "/>
            </el-upload>
            <quill-editor
              ref="QuillEditor2"
              v-model="content2"
...
export default {
  ...
  methods: {
    ...
    isBonus(e) {
        if(e=="1") {
          this.has_sub_commission = true
        }else {
          this.has_sub_commission = false
        }
    },
    goodsKind() {
      if (this.goodstype == 0) {
    ...
    createGoods() {
      ...
      request({
        ...
        data: {
          ...
          sbc_id: self.sbc_id[1],
          three_sbc_id: self.sbc_id[2],
          cate_list: cate_list,
          ...
          is_vip: self.isVIP,
          place_origin: self.place,
          yield: self.output,
          sub_commission: self.sub_commission,
    ...
    getDetail() {
      ...
      request({
        ...
        self.place = self.detail.place_origin
        self.output  = self.detail.yield
        self.sub_commission = self.detail.sub_commission
        self.quality = self.detail.quality
        self.discount = self.detail.discount
        self.goodsOrd = self.detail.sort
        self.is_support = self.detail.is_support
