          ...
          <div style="margin:20px">
            <el-popover
              ...
              <div class="edit-panel1">
                ...
                <div class="one-row1">
                    <div class="one-roe-title">banner管理</div>
                    <div>
                        <el-button type="success" size="mini" @click="addBanner">新增banner</el-button>
                    </div>
                    <div>
                         <el-table
                            :data="tableData"
                            ...
                            <template slot-scope="scope">
                                <img :src="scope.row.banner_img" width="120" height="60" style="object-fit: contain;">
                            </template>
                            </el-table-column>
                            <el-table-column
                            label="姓名"
                            ...
                            <el-table-column
                            label="手机号"
                            prop="activity_remark"/>
                            <!-- <el-table-column
                            label="跳转方式">
                                <template slot-scope="scope">
                                    <div>{{scope.row.jump_type==8 ? '外链' : scope.row.jump_type==9 ? '装修页' : '-'}}</div>
                                </template>
                            </el-table-column> -->
                            <el-table-column
                            label="银行卡号"
                            ...
                            >
                                <template slot-scope="scope">
                                    <div>{{scope.row.is_show==0 ? '不显示' : '显示'}}</div>
                                </template>
                            </el-table-column>
                            <el-table-column
                            align="right"
                            label="操作">
                            <template slot-scope="scope">
                                <el-button @click="handleClick(scope.row)" type="text" icon="el-icon-edit" size="small">编辑</el-button>
                                <el-button type="text" size="small" icon="el-icon-delete" @click="detectClick(scope.row)">删除</el-button>
                            </template>
                            </el-table-column> -->
                        </el-table>
                         <el-pagination
...
export default {
  ...
  methods: {
    // 编辑
    handleClick(row) {
        this.$router.push({path:'/shop_tpl_active',query: {id:row.id}})
    },
    // 删除
    detectClick(row) {
      let self = this
      ...
      this.$confirm('操作将删除此优惠券, 是否继续?', '提示', {
        ...
        request({
          method: 'post',
          url: '/item/item-details-activity-list/delete-activity-list',
          data: {
              id: row.id
          }
        }).then(function(res) {
    ...
    // 新增banner
    addBanner() {
        this.$router.push('/shop_tpl_active')
    },
    xianshiC(in1, in2, d) {
    ...
    getActive() {
      ...
      request({
        method: 'post',
        url: '/item/item-details-activity-list/get-all-activity-list',
        data: {
          start_page: self.start_page4,
          pages: 10
        }
      }).then(function(res) {
        // console.log(111,res)
        if(res.data.errcode==0) {
            self.total_pages4 = parseInt(res.data.data.total_pages)
        ...
        //   self.content.list[0].type=res.data.data.jump_type
